...
type Seller={id:string;userId:string;name:string;email:string;sellerType:string;countryCode:string;verified:boolean;suspended:boolean;activatedAt:string;suspendedAt:string|null;suspensionReason:string|null;listingCount:number};
type User={id:string;name:string;location:{countryCode:string|null;state:string;stateCode:string;city:string;district:string|null}};
...
export default function SellerPage(){
  const {sellerId}=useParams<{sellerId:string}>(); const {toast,listings,subscriptions,markets}=useAdminData(); const [seller,setSeller]=useState<Seller|null>(null); const [owner,setOwner]=useState<User|null>(null); const [loading,setLoading]=useState(true);
  ...
  const sellerMarket=markets.find(m=>m.code===seller.countryCode); const status=seller.suspended?"Suspended":releaseFeatures.cpfVerification&&seller.verified?"Verified":"Active"; const sellerListings=listings.filter((x)=>x.publicSellerId===seller.id); const subscription=subscriptions.find((x)=>x.sellerId===seller.id);
