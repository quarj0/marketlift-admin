...
export default function SubscriptionsPage() {
  ...
  const { plans, subscriptions, saveSellerPlan, markets, sellerPlanMarketPrices, canAccess } = useAdminData();
  ...
  const enabledMarkets=markets.filter(m=>m.isEnabled);
  ...
  return <div className="space-y-6">
    <PageHeader title="Subscriptions" description="Manage seller plan features and review subscriptions. Country-specific pricing is managed under Markets." actions={<div className="flex flex-wrap gap-2">{canAccess("markets")&&<SafeLink href="/markets" className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-3.5 text-sm font-bold text-slate-700 hover:bg-slate-50"><Icons.globe size={15}/> Market pricing</SafeLink>}<AdminButton onClick={openNew}>+ Create plan</AdminButton></div>} />

    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {plans.map((item)=>{const priced=enabledMarkets.filter(m=>sellerPlanMarketPrices.some(p=>p.marketCode===m.code&&p.planId===item.id&&p.active)).length;return <article key={item.id} className="relative rounded-xl border border-slate-200 bg-white p-5 transition hover:shadow-lg hover:shadow-slate-950/5">
        ...
        <div className="mt-4"><strong className="text-lg font-black text-slate-950">Market-priced</strong><p className="mt-1 text-[10px] text-slate-400">Offered in {priced}/{enabledMarkets.length||0} enabled markets</p></div>
        <div className="mt-5 space-y-3 border-t border-slate-100 pt-4 text-xs"><Row label="Current sellers" value={item.sellers.toLocaleString()}/><Row label="Listing limit" value={String(item.listings)}/><Row label="Promotion credits" value={String(item.featured)}/><Row label="Status" value={item.active?"Active":"Inactive"}/></div>
        <button type="button" onClick={()=>openEdit(item)} className="mt-5 w-full rounded-lg border border-slate-200 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2">Edit plan features</button>
      </article>})}
    </div>
    ...
    <Dialog open={editing!==null} onClose={()=>setEditing(null)} title={editing==="new"?"Create subscription plan":`Edit ${editing?.name??"plan"}`} description="Configure plan features and limits. Set prices independently for each country under Markets." footer={<><AdminButton variant="outline" onClick={()=>setEditing(null)}>Cancel</AdminButton><AdminButton disabled={busy||form.name.trim().length<2} onClick={()=>void save()}>{busy?"Saving…":"Save plan"}</AdminButton></>}>
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Plan name" value={form.name} onChange={v=>setForm(f=>({...f,name:v}))}/><Field label="Listing limit" type="number" value={form.listingLimit} onChange={v=>setForm(f=>({...f,listingLimit:v}))}/><Field label="Promotion credits" type="number" value={form.promotionCredits} onChange={v=>setForm(f=>({...f,promotionCredits:v}))}/><Field label="Visibility weight" type="number" value={form.visibilityWeight} onChange={v=>setForm(f=>({...f,visibilityWeight:v}))}/>
        <div className="sm:col-span-2 rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-900">Plan prices are intentionally not edited here. Configure monthly/yearly prices separately for Ghana, Nigeria, Brazil and every other enabled market in <SafeLink href="/markets" className="font-black underline">Markets</SafeLink>.</div>
...
function Field({label,value,onChange,type="text"}:{label:string;value:string;onChange:(v:string)=>void;type?:string}){return <label><span className="mb-2 block text-xs font-bold text-slate-700">{label}</span><input type={type} min={type==="number"?0:undefined} step="1" value={value} onChange={e=>onChange(e.target.value)} className="h-10 w-full rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10"/></label>}
