...
export default function PromotionsPage(){
  const {payments,listings,promotionProducts,promotionMarketPrices,markets,canAccess}=useAdminData();
  const listingById=new Map(listings.map(l=>[l.id,l])); const productById=new Map(promotionProducts.map(p=>[p.id,p]));
  const rows=payments.filter(p=>p.purpose.toLowerCase().includes("promotion")||Boolean(p.promotionId));
  return <div className="space-y-6"><PageHeader title="Promotions" description="Review promotion products and seller promotion purchases. Prices are configured per country." actions={canAccess("markets")?<SafeLink href="/markets" className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3.5 text-sm font-bold text-slate-700 hover:bg-slate-50"><Icons.globe size={15}/> Market pricing</SafeLink>:undefined}/>
    <div className="grid gap-4 lg:grid-cols-3">{promotionProducts.map((product,index)=>{const Icon=iconCycle[index%iconCycle.length];const activeMarkets=markets.filter(m=>m.isEnabled&&promotionMarketPrices.some(p=>p.marketCode===m.code&&p.promotionId===product.id&&p.active));return <Promo key={product.id} title={product.name} availability={`${activeMarkets.length}/${markets.filter(m=>m.isEnabled).length} enabled markets`} description={product.description||`Runs for ${product.durationDays} day${product.durationDays===1?"":"s"}.`} icon={<Icon size={20}/>} duration={product.durationDays}/>})}</div>
