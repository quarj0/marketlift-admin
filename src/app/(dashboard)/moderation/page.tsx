...
export default function ModerationPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Moderation"
        description="Review flagged listings and enforce marketplace policy consistently."
      />
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-amber-800/70">
              Approve and reject are mutually exclusive final outcomes for the
              current moderation case. Rejection requires an internal reason for
              the audit trail.
            </p>
          </div>
        </div>
      ...
      <DataTable
        rows={flagged}
        searchText={(listing) =>
          `${listing.id} ${listing.title} ${listing.seller} ${listing.category}`
        }
        ...
        columns={[
          {
            ...
            cell: (listing) => (
              <div>
                <div className="max-w-75 truncate font-bold text-slate-900">
                  {listing.title}
                </div>
          ...
          },
          {
            key: "category",
            label: "Category",
            cell: (listing) => listing.category,
          },
          {
            key: "reports",
            label: "Reports",
            cell: (listing) => (
              <span className="font-black text-red-600">{listing.reports}</span>
            ),
          },
          {
            key: "status",
            ...
            cell: (listing) => (
              <StatusBadge
                status={getStatus("listing", listing.id, listing.status)}
              />
            ),
          },
          {
            ...
            cell: (listing) => {
              ...
              const moderationFinal =
                decision?.action ??
                (currentStatus === "Rejected" ? "Rejected" : null);
              const canRemove = !["Rejected", "Removed", "Deleted"].includes(
                currentStatus,
              );
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {moderationFinal ? (
                    <span className="inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 text-[11px] font-black text-slate-700">
                      <Icons.lock size={13} aria-hidden="true" /> Moderation:{" "}
                      {moderationFinal}
                    </span>
                    ...
                    <span className="inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2.5 text-[11px] font-black text-red-800">
                      <Icons.lock size={13} aria-hidden="true" /> Enforcement:
                      Removed
                    </span>
                  ) : currentStatus === "Deleted" ? (
                    <span className="inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 text-[11px] font-black text-slate-700">
                      <Icons.lock size={13} aria-hidden="true" /> Lifecycle:
                      Deleted by seller
                    </span>
                  ) : (
                    <>
                      <ActionDialog
                        trigger={
                          <button className="rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[11px] font-black text-emerald-700 hover:bg-emerald-100">
                            Approve
                          </button>
                        }
                      ...
                      <ActionDialog
                        trigger={
                          <button className="rounded-lg bg-red-50 px-2.5 py-1.5 text-[11px] font-black text-red-700 hover:bg-red-100">
                            Reject
                          </button>
                        }
