...
export type SubscriptionRecord = { id:string; sellerId:string; sellerName:string; planName:string; billingCycle:string; status:Status; periodEnd:string; promotionCreditsRemaining:number };
export type PromotionProductRecord = { id:string; name:string; description:string; durationDays:number; price:string; priceValue:number };
export type ActivityRecord = { id:string; admin:string; action:string; target:string; time:string; ip:string; targetType:string; targetId:string };
