...
import type {
  ...
  PromotionProductRecord, Status, SubscriptionRecord, SupportTicketRecord, VerificationSummary,
} from "@/types/admin";
...
type ApiLocation={state?:string;stateCode?:string;city?:string;district?:string|null};
...
type SellerQueryRecord={id:string;userId:string;name:string;email:string;sellerType:string;verified:boolean;suspended:boolean;activatedAt:string;suspendedAt:string|null;suspensionReason:string|null;listingCount:number};
...
type VerificationQueryData={verifications:{id:string;sellerId:string;sellerName:string;cpfMasked:string;legalName:string;birthDate:string;documentType:string|null;documentFrontUrl:string|null;documentBackUrl:string|null;selfieUrl:string|null;status:string;riskLevel:string;riskFlags:string[];submittedAt:string;decisionNote:string|null}[];verificationQueueSummary:VerificationSummary};
...
export function AdminDataProvider({children}:{children:ReactNode}) {
  ...
  const refresh=useCallback(async()=>{
    ...
    try {
      ...
      const [dashboardRes,notificationRes,usersRes,sellerRes,billingRes,listingRes,moderationRes,reportRes,verificationRes,paymentRes,supportRes,categoryRes,auditRes,promotionRes]=await Promise.all([
      ...
      const plans:PlanRecord[]=(billingRes?.adminSellerPlans||[]).map((p)=>({id:p.id,name:p.name,price:money(Number(p.monthlyPrice)),priceValue:Number(p.monthlyPrice),yearlyPrice:money(Number(p.yearlyPrice)),yearlyPriceValue:Number(p.yearlyPrice),period:"/month",sellers:rawSubs.filter((s)=>s.plan.id===p.id&&s.status==="active").length,listings:p.listingLimit,featured:p.promotionCredits,badge:p.recommended?"Recommended":"",active:Boolean(p.active),visibilityWeight:Number(p.visibilityWeight),recommended:Boolean(p.recommended),features:p.features||[]}));
      ...
      const promotionProducts:PromotionProductRecord[]=(promotionRes?.promotionOptions||[]).map((p)=>({id:p.id,name:p.name,description:p.description,durationDays:p.durationDays,price:money(Number(p.price)),priceValue:Number(p.price)}));
  ...
  const value=useMemo<ContextValue>(()=>({...data,sessionUser,loading,error,refresh,canAccess,getStatus,setStatus,getDecision,commitDecision,isCategoryDeleted:(id)=>!data.categories.some((c)=>c.id===id),deleteCategory,saveSellerPlan,replySupportTicket,markNotificationRead,markAllNotificationsRead,toast}),[data,sessionUser,loading,error,refresh,canAccess,getStatus,setStatus,getDecision,commitDecision,deleteCategory,saveSellerPlan,replySupportTicket,markNotificationRead,markAllNotificationsRead,toast]);
