...
function uiStatus(value?:string|null):Status {
  ...
  const map:Record<string,Status>={active:"Active",published:"Active",verified:"Verified",pending:"Pending",under_review:"Review",review:"Review",rejected:"Rejected",removed:"Removed",resolved:"Resolved",dismissed:"Dismissed",open:"Open",paid:"Paid",failed:"Failed",refunded:"Refunded",expired:"Expired",draft:"Draft",paused:"Paused",sold:"Sold",suspended:"Suspended",cancelled:"Cancelled",closed:"Closed",past_due:"Past due"};
...
export function AdminDataProvider({children}:{children:ReactNode}) {
  ...
  const getDecision=useCallback((kind:string,id:string):Decision|undefined=>{
    if(kind==="listing") { const c=data.moderationCases.find((m)=>m.listingId===id); if(c&&["approved","rejected"].includes(c.status)) return {action:titleCase(c.status),status:c.status==="approved"?"Active":"Rejected",at:c.decidedAt}; const l=data.listings.find((x)=>x.id===id); if(l?.status==="Removed") return {action:"Removed",status:"Removed",at:l.created}; }
