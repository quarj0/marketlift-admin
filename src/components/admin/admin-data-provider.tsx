...
type AdminData = {
  ...
  promotionProducts: PromotionProductRecord[];
  activityLog: ActivityRecord[];
  moderationCases: ModerationRecord[];
  paymentSummary: PaymentSummary;
  verificationSummary: VerificationSummary;
  dashboard: AdminDashboardRecord;
  notifications: AdminNotificationRecord[];
  unreadNotificationCount: number;
  markets: AdminMarketRecord[];
  sellerPlanMarketPrices: SellerPlanMarketPriceRecord[];
  promotionMarketPrices: PromotionMarketPriceRecord[];
};
...
type ContextValue = AdminData & {
  ...
  canAccess: (area: AdminArea) => boolean;
  ...
  setSellerPlanMarketPrice: (input: {
    marketCode: string;
    planId: string;
    monthlyPrice: number;
    yearlyPrice: number;
...
const locationText = (loc?: {
  ...
  [loc?.district, loc?.city, loc?.stateCode || loc?.state]
    .filter(Boolean)
    .join(", ") || "—";
...
async function safeQuery<T>(
  ...
  try {
    ...
    if (
      error instanceof MarketliftApiError &&
...
type SubscriptionQueryRecord = {
  ...
  promotionCreditsRemaining: number;
  plan: SellerPlanQueryRecord;
};
type BillingQueryData = {
  adminSubscriptions: SubscriptionQueryRecord[];
...
type VerificationQueryData = {
  ...
  verificationQueueSummary: VerificationSummary;
};
...
type MarketQueryData = {
  adminMarkets: AdminMarketRecord[];
  adminSellerPlanMarketPrices: SellerPlanMarketPriceRecord[];
  adminPromotionMarketPrices: PromotionMarketPriceRecord[];
};
...
export function AdminDataProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const rawReports = reportRes?.reports || [];
      const markets = (marketRes?.adminMarkets || [])
        .slice()
        .sort(
          (a, b) =>
      ...
      const defaultMarket =
        markets.find((m) => m.isDefault) ||
        markets.find((m) => m.isEnabled) ||
        markets[0];
      const subscriptions: SubscriptionRecord[] = rawSubs.map((s) => ({
        id: s.id,
        sellerId: s.sellerId,
        sellerName: s.sellerName,
        planName: s.plan.name,
        billingCycle: titleCase(s.billingCycle),
        status: uiStatus(s.status),
        periodEnd: fmtDate(s.currentPeriodEnd),
        promotionCreditsRemaining: s.promotionCreditsRemaining,
      }));
      ...
      const users: AdminUserRecord[] = rawUsers.map((u) => ({
        ...
        type: u.staff
          ...
          : u.sellerId
            ? "Buyer & Seller"
            : "Buyer",
        status: u.suspended ? "Suspended" : u.active ? "Active" : "Pending",
        joined: fmtDay(u.joinedAt),
        location: locationText(u.location || undefined),
        avatar: initials(u.name),
        sellerId: u.sellerId || null,
        adminRole: u.adminRole || null,
      }));
      const sellers: AdminSellerRecord[] = (sellerRes?.adminSellers || []).map(
        (s) => {
          const u = userById.get(s.userId);
          const sub = subBySeller.get(s.id);
          return {
            id: s.id,
            publicSellerId: s.id,
            userId: s.userId,
            ...
            countryCode:
              s.countryCode ||
              u?.location?.countryCode ||
              defaultMarket?.code ||
              "",
          };
        },
      );
      const reports: AdminReportRecord[] = rawReports.map((r) => ({
        id: r.id,
        reference: r.reference,
        type: titleCase(r.targetType),
      ...
      rawReports
        ...
        .forEach((r) =>
          reportCountByListing.set(
            r.targetId,
            (reportCountByListing.get(r.targetId) || 0) + 1,
          ),
        );
      const listings: AdminListingRecord[] = rawListings.map((l) => {
        const listingMarket =
          marketByCode.get(l.location?.countryCode || "") || defaultMarket;
        return {
          id: l.id,
          publicId: l.id,
          publicSlug: l.slug,
          publicSellerId: l.seller.id,
          title: l.title,
          ...
          images: (l.images || []).map(mediaUrl),
          image: mediaUrl(l.images?.[0]),
          description: l.description,
          condition: l.condition || "—",
          location: locationText(l.location || undefined),
          status: l.sellerDeletedAt ? "Deleted" : uiStatus(l.status),
          rawStatus: l.status,
      ...
      rawListings.forEach((l) =>
        listingCounts.set(l.category, (listingCounts.get(l.category) || 0) + 1),
      );
      const categories: CategoryRecord[] = (
        categoryRes?.adminCategories || []
      ).map((c) => ({
        id: c.id,
        name: c.name,
        slug: c.id,
      ...
      const verifications: AdminVerificationRecord[] = (
        verificationRes?.verifications || []
      ).map((v) => {
        const vm = marketByCode.get(v.identityCountryCode || "");
        return {
          id: v.id,
          sellerId: v.sellerId,
          seller: v.sellerName,
          owner: v.legalName || v.sellerName,
          document: v.documentType ? titleCase(v.documentType) : "Not provided",
          submitted: fmtDate(v.submittedAt),
          status: uiStatus(v.status),
          risk: titleCase(v.riskLevel),
          identityCountryCode: v.identityCountryCode || "",
          identityType: v.identityType || "",
          identityMasked: v.identityMasked || v.cpfMasked || "",
          identityLabel:
            vm?.identityLabel || titleCase(v.identityType || "Identity"),
          cpfMasked: v.cpfMasked || "",
          legalName: v.legalName,
          birthDate: v.birthDate,
          documentFrontUrl: mediaUrl(v.documentFrontUrl),
          documentBackUrl: mediaUrl(v.documentBackUrl),
          selfieUrl: mediaUrl(v.selfieUrl),
          flags: v.riskFlags || [],
          decisionNote: v.decisionNote || "",
        };
      });
      const payments: AdminPaymentRecord[] = (
        paymentRes?.adminPayments || []
      ).map((p) => ({
        id: p.id,
        ...
        amount: money(
          Number(p.amount),
          p.currency || defaultMarket?.currency || "GHS",
          marketByCode.get(
            (sellerRes?.adminSellers || []).find((s) => s.id === p.sellerId)
              ?.countryCode || "",
          )?.locale,
        ),
        amountValue: Number(p.amount),
        currency: p.currency || defaultMarket?.currency || "GHS",
      ...
      const plans: PlanRecord[] = (billingRes?.adminSellerPlans || []).map(
        (p) => ({
          id: p.id,
          name: p.name,
          price: "Market-priced",
          priceValue: Number(p.monthlyPrice),
          yearlyPrice: "Market-priced",
          yearlyPriceValue: Number(p.yearlyPrice),
          period: "",
          sellers: rawSubs.filter(
            (s) => s.plan.id === p.id && s.status === "active",
          ).length,
          listings: p.listingLimit,
          featured: p.promotionCredits,
          badge: p.recommended ? "Recommended" : "",
          active: Boolean(p.active),
          visibilityWeight: Number(p.visibilityWeight),
          recommended: Boolean(p.recommended),
          features: p.features || [],
        }),
      );
      const activityLog: ActivityRecord[] = (auditRes?.auditEvents || []).map(
        (a) => ({
          id: a.id,
          admin: a.actorName || "System",
          action: titleCase(String(a.action).replace(/[.]/g, " ")),
      ...
      const promotionProducts: PromotionProductRecord[] = (
        promotionRes?.promotionOptions || []
      ).map((p) => ({
        id: p.id,
        name: p.name,
        ...
        price: defaultMarket
          ? money(Number(p.price), defaultMarket.currency, defaultMarket.locale)
          : String(p.price),
        priceValue: Number(p.price),
      }));
      const moderationCases: ModerationRecord[] = (
        moderationRes?.moderationQueue || []
      ).map((m) => ({
        id: m.id,
        status: m.status,
        source: m.source,
        ...
        openedAt: fmtDate(m.openedAt),
        decidedAt: fmtDate(m.decidedAt),
      ...
      const notifications: AdminNotificationRecord[] = (
        ...
        .filter((n) => n.data?.adminOperational === true)
        .map((n) => ({
          id: n.id,
          type: n.type,
          title: n.title,
      ...
      setData({
        ...
        activityLog,
        moderationCases,
        paymentSummary: paymentRes?.paymentSummary || emptySummary,
        verificationSummary:
          verificationRes?.verificationQueueSummary || emptyVerificationSummary,
        dashboard: dashboardRes?.adminDashboard || emptyDashboard,
        notifications,
        unreadNotificationCount: notifications.filter((item) => !item.read)
          .length,
        markets,
        sellerPlanMarketPrices: marketRes?.adminSellerPlanMarketPrices || [],
    ...
    async (
      ...
      try {
        ...
        if (kind === "user")
          await mutate(
            status === "Suspended"
              ? `mutation($id:ID!,$reason:String!){suspendAccount(userId:$id,reason:$reason){id}}`
              : `mutation($id:ID!,$reason:String!){reactivateAccount(userId:$id,reason:$reason){id}}`,
            { id, reason: safeReason },
          );
        else if (kind === "seller")
          await mutate(
            status === "Suspended"
              ? `mutation($id:ID!,$reason:String!){suspendSeller(sellerId:$id,reason:$reason){id}}`
              : `mutation($id:ID!,$reason:String!){restoreSeller(sellerId:$id,reason:$reason){id}}`,
            { id, reason: safeReason },
          );
        else if (kind === "listing" && status === "Review")
          await mutate(
            `mutation($id:ID!,$reason:String!){moveListingToReview(listingId:$id,reason:$reason){id}}`,
            { id, reason: safeReason },
          );
        else if (kind === "report" && status === "Review")
          await mutate(
            `mutation($id:ID!,$note:String!){moveReportToReview(reportId:$id,note:$note){id}}`,
            { id, note: safeReason },
          );
        else if (kind === "verification" && status === "Review")
          await mutate(
            `mutation($id:ID!,$note:String!){moveVerificationToReview(id:$id,note:$note){id}}`,
            { id, note: safeReason },
          );
        else if (kind === "ticket")
          await mutate(
            `mutation($id:ID!,$status:String!){adminUpdateSupportTicket(ticketId:$id,status:$status){id}}`,
            { id, status: status.toLowerCase().replace(/ /g, "_") },
          );
    ...
    async (
      ...
      try {
        ...
        if (kind === "listing") {
          if (action === "Approved")
            await mutate(
              `mutation($id:ID!,$reason:String!){approveListing(listingId:$id,reason:$reason){id}}`,
              { id, reason: safeReason },
            );
          else if (action === "Rejected")
            await mutate(
              `mutation($id:ID!,$reason:String!){rejectListing(listingId:$id,reason:$reason){id}}`,
              { id, reason: safeReason },
            );
          else if (action === "Removed")
            await mutate(
              `mutation($id:ID!,$reason:String!){removeListing(listingId:$id,reason:$reason){id}}`,
              { id, reason: safeReason },
            );
        } else if (kind === "verification") {
          if (action === "Approved")
            await mutate(
              `mutation($id:ID!,$note:String!){approveVerification(id:$id,note:$note){id}}`,
              { id, note: safeReason },
            );
          else
            await mutate(
              `mutation($id:ID!,$note:String!){rejectVerification(id:$id,note:$note){id}}`,
              { id, note: safeReason },
            );
        } else if (kind === "report") {
          if (action === "Resolved")
            await mutate(
              `mutation($id:ID!,$reason:String!){resolveReport(reportId:$id,reason:$reason){id}}`,
              { id, reason: safeReason },
            );
          else
            await mutate(
              `mutation($id:ID!,$reason:String!){dismissReport(reportId:$id,reason:$reason){id}}`,
              { id, reason: safeReason },
            );
        ...
        setDecisionOverrides((current) => ({
          ...current,
          [`${kind}:${id}`]: { action, status, at: new Date().toISOString() },
        }));
        toast(
          message || `${id} ${action.toLowerCase()}`,
          `Final status: ${status}.`,
          tone,
        );
        return true;
  ...
  const getStatus = useCallback(
    (kind: string, id: string, fallback: string) => {
      const override = statusOverrides[`${kind}:${id}`];
      if (override) return override;
      const collections: Record<string, { id: string; status?: string }[]> = {
        user: data.users,
        seller: data.sellers,
        listing: data.listings,
        report: data.reports,
        verification: data.verifications,
        ticket: data.supportTickets,
        payment: data.payments,
      };
      return (
        collections[kind]?.find((item) => item.id === id)?.status || fallback
      );
    },
    [data, statusOverrides],
  );
  const getDecision = useCallback(
    (kind: string, id: string): Decision | undefined => {
      const override = decisionOverrides[`${kind}:${id}`];
      if (override) return override;
      if (kind === "listing") {
        const c = data.moderationCases.find((m) => m.listingId === id);
        if (c && ["approved", "rejected"].includes(c.status))
          return {
            action: titleCase(c.status),
            status: c.status === "approved" ? "Active" : "Rejected",
            at: c.decidedAt,
          };
      }
      if (kind === "verification") {
        const v = data.verifications.find((x) => x.id === id);
        if (v && ["Verified", "Rejected"].includes(v.status))
          return {
            action: v.status === "Verified" ? "Approved" : "Rejected",
            status: v.status,
            at: v.submitted,
          };
      }
      if (kind === "report") {
        ...
        if (r && ["Resolved", "Dismissed"].includes(r.status))
          return { action: r.status, status: r.status, at: r.created };
      }
      return undefined;
    },
  ...
  const deleteCategory = useCallback(
    async (id: string, name: string) => {
      try {
        await mutate(
          `mutation($id:String!){deleteCategory(categoryId:$id){slug affectedListings}}`,
          { id },
        );
        toast(
          "Category deleted",
          `${name} was permanently removed from the catalog.`,
          "danger",
        );
        return true;
      } catch (e) {
  ...
  const saveSellerPlan = useCallback(
    async (input: {
      id?: string;
      name: string;
      monthlyPrice: number;
      ...
      try {
        if (input.id)
          await mutate(
            `mutation($id:String!,$name:String!,$monthlyPrice:Float!,$yearlyPrice:Float!,$listingLimit:Int!,$promotionCredits:Int!,$visibilityWeight:Float!,$recommended:Boolean!,$features:[String!]!,$active:Boolean!){updateSellerPlan(id:$id,name:$name,monthlyPrice:$monthlyPrice,yearlyPrice:$yearlyPrice,listingLimit:$listingLimit,promotionCredits:$promotionCredits,visibilityWeight:$visibilityWeight,recommended:$recommended,features:$features,active:$active){id}}`,
            input,
          );
        else
          await mutate(
            `mutation($name:String!,$monthlyPrice:Float!,$yearlyPrice:Float!,$listingLimit:Int!,$promotionCredits:Int!,$visibilityWeight:Float!,$recommended:Boolean!,$features:[String!],$active:Boolean!){createSellerPlan(name:$name,monthlyPrice:$monthlyPrice,yearlyPrice:$yearlyPrice,listingLimit:$listingLimit,promotionCredits:$promotionCredits,visibilityWeight:$visibilityWeight,recommended:$recommended,features:$features,active:$active){id}}`,
            input,
          );
        toast(
          input.id ? "Plan updated" : "Plan created",
          `${input.name} is now saved in the seller plan catalog.`,
        );
        return true;
      } catch (e) {
        toast(
  ...
  const updateMarket = useCallback(
    async (
      ...
      try {
        await mutate(
          `mutation($code:String!,$input:MarketConfigurationInput!){updateMarket(code:$code,input:$input){code isEnabled isDefault pricingReady}}`,
          { code, input },
        );
        toast("Market updated", `${code} configuration is live.`);
        return true;
      } catch (e) {
        toast(
          "Market could not be updated",
          e instanceof Error ? e.message : undefined,
          "danger",
        );
        return false;
  ...
  const setSellerPlanMarketPrice = useCallback(
    async (input: {
      marketCode: string;
      planId: string;
      monthlyPrice: number;
      ...
      try {
        await mutate(
          `mutation($marketCode:String!,$planId:String!,$monthlyPrice:Float!,$yearlyPrice:Float!,$active:Boolean!){setSellerPlanMarketPrice(marketCode:$marketCode,planId:$planId,monthlyPrice:$monthlyPrice,yearlyPrice:$yearlyPrice,active:$active){marketCode planId monthlyPrice yearlyPrice active}}`,
          input,
        );
        toast("Plan price updated", `${input.marketCode} pricing is saved.`);
        return true;
      } catch (e) {
        toast(
          "Plan price could not be saved",
          e instanceof Error ? e.message : undefined,
          "danger",
        );
        return false;
  ...
  const setPromotionMarketPrice = useCallback(
    async (input: {
      marketCode: string;
      promotionId: string;
      price: number;
      ...
      try {
        await mutate(
          `mutation($marketCode:String!,$promotionId:String!,$price:Float!,$active:Boolean!){setPromotionMarketPrice(marketCode:$marketCode,promotionId:$promotionId,price:$price,active:$active){marketCode promotionId price active}}`,
          input,
        );
        toast(
          "Promotion price updated",
          `${input.marketCode} pricing is saved.`,
        );
        return true;
      } catch (e) {
        toast(
          "Promotion price could not be saved",
          e instanceof Error ? e.message : undefined,
          "danger",
        );
        return false;
  ...
  const replySupportTicket = useCallback(
    async (id: string, message: string) => {
      try {
        await mutate(
          `mutation($id:ID!,$message:String!){adminReplySupportTicket(ticketId:$id,message:$message,status:"review"){id}}`,
          { id, message },
        );
        toast("Reply sent", `Ticket ${id} was updated.`);
        return true;
      } catch (e) {
        toast(
  ...
  const markNotificationRead = useCallback(
    async (id: string) => {
      const current = data.notifications.find((item) => item.id === id);
      if (!current || current.read) return;
      try {
        await graphqlRequest(
          `mutation($id:ID!){markNotificationRead(notificationId:$id)}`,
          { id },
        );
        setData((state) => ({
          ...state,
  ...
  const value = useMemo<ContextValue>(
    () => ({
      ...
      getDecision,
      commitDecision,
      isCategoryDeleted: (id) => !data.categories.some((c) => c.id === id),
      deleteCategory,
      saveSellerPlan,
      updateMarket,
    ...
    <AdminDataContext.Provider value={value}>
      ...
      >
        {toasts.map((item) => (
          <div
            key={item.id}
            className={`pointer-events-auto rounded-xl border bg-white p-4 shadow-2xl shadow-slate-950/10 ${item.tone === "danger" ? "border-red-200" : item.tone === "info" ? "border-blue-200" : "border-emerald-200"}`}
          >
            <div className="flex items-start gap-3">
              <span
                className={`mt-0.5 grid size-6 shrink-0 place-items-center rounded-full text-xs font-black ${item.tone === "danger" ? "bg-red-50 text-red-700" : item.tone === "info" ? "bg-blue-50 text-blue-700" : "bg-emerald-50 text-emerald-700"}`}
              >
                {item.tone === "danger"
                  ? "!"
                  : item.tone === "info"
