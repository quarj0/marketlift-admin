...
const sections:{label:string;items:Item[]}[] = [
  ...
  { label: "Marketplace", items: [
    ...
    { label: "Sellers", href: "/sellers", icon: Icons.store, area:"sellers" },
    { label: "Listings", href: "/listings", icon: Icons.tag, area:"listings" },
    { label: "Categories", href: "/categories", icon: Icons.layers, area:"categories" },
  ]},
...
export function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const environment=(process.env.NEXT_PUBLIC_MARKETLIFT_ENVIRONMENT||process.env.NODE_ENV||"development").replace(/\b\w/g,(c)=>c.toUpperCase());
  async function logout(){try{await apiRequest<void>("/api/v1/auth/logout/",{method:"POST"});onNavigate?.();router.replace("/login");router.refresh();}catch(e){toast("Sign out failed",e instanceof Error?e.message:undefined,"danger");}}
  return <aside aria-label="Marketlift administration" className="flex h-full w-[272px] flex-col bg-[#0b1512] text-slate-300">
    <div className="flex h-[74px] items-center border-b border-white/8 px-5">
      <SafeLink href="/dashboard" className="flex items-center gap-3" onClick={onNavigate}>
        <div className="grid size-9 place-items-center rounded-xl bg-emerald-500 text-white shadow-lg shadow-emerald-950/30"><svg viewBox="0 0 32 32" className="size-6" fill="none"><path d="M7 22V10h4l5 6 5-6h4v12h-4v-6l-5 6-5-6v6H7Z" fill="currentColor"/></svg></div>
        <div><div className="text-[17px] font-black tracking-tight text-white">Marketlift</div><div className="text-[9px] font-bold uppercase tracking-[.24em] text-emerald-400">Admin console</div></div>
      </SafeLink>
    </div>
    ...
    <div className="border-t border-white/8 p-3">
      <div className="mb-2 rounded-xl bg-white/[.035] px-3 py-2.5"><div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase tracking-wide text-slate-600">Environment</span><span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-400"><i aria-hidden="true" className="size-1.5 rounded-full bg-emerald-400"/>{environment}</span></div></div>
      <button type="button" onClick={()=>void logout()} className="flex h-10 w-full items-center gap-3 rounded-lg px-3 text-[13px] font-semibold text-slate-500 transition hover:bg-white/5 hover:text-slate-300"><Icons.logout size={18}/> Sign out</button>
    </div>
  </aside>;
}
