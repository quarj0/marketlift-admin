...
import { useTableQuery } from "./table-query-context";
...
export function DataTable<T extends { id: string }>({
  ...
  const remote = useTableQuery();
  const [localQuery, setLocalQuery] = useState("");
  const [localStatus, setLocalStatus] = useState("all");
  const [localPage, setLocalPage] = useState(1);
  const [localSize, setLocalSize] = useState(5);
  const query = remote?.query ?? localQuery;
  const status = remote?.status ?? localStatus;
  const page = remote?.page ?? localPage;
  const pageSize = remote?.pageSize ?? localSize;
  const setQuery = (value: string) => { setSelected([]); if (remote) remote.update({ query: value, page: 1 }); else { setLocalQuery(value); setLocalPage(1); } };
  const setStatus = (value: string) => { setSelected([]); if (remote) remote.update({ status: value, page: 1 }); else { setLocalStatus(value); setLocalPage(1); } };
  const setPage = (value: number) => { setSelected([]); if (remote) remote.update({ page: value }); else setLocalPage(value); };
  const setPageSize = (value: number) => { setSelected([]); if (remote) remote.update({ pageSize: value, page: 1 }); else { setLocalSize(value); setLocalPage(1); } };
  const [selected, setSelected] = useState<string[]>([]);
  const searchId = useId();

  const filtered = useMemo(() => remote ? rows : rows.filter((row) => {
  ...
  }), [rows, query, status, searchText, statusOf, remote]);

  const total = remote?.total ?? filtered.length;
  const pages = Math.max(1, Math.ceil(total / pageSize));
  const currentPage = Math.min(page, pages);
  const start = (currentPage - 1) * pageSize;
  const visible = remote?.loading ? [] : remote ? filtered : filtered.slice(start, start + pageSize);
  ...
  return <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,.02)]">
    <div className="flex flex-col gap-3 border-b border-slate-200 p-4 xl:flex-row xl:items-center xl:justify-between">
      <div className="relative w-full xl:max-w-md">
        ...
        <input id={searchId} type="search" value={query} onChange={(event) => { setQuery(event.target.value); }} className="h-10 w-full rounded-lg border border-slate-300 bg-white pl-9 pr-9 text-sm text-slate-900 outline-none transition placeholder:text-slate-500 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20" placeholder={searchPlaceholder}/>
        {query && <button type="button" onClick={() => { setQuery(""); }} className="absolute right-2.5 top-1/2 grid size-7 -translate-y-1/2 place-items-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-800" aria-label={`Clear ${selectedLabel} search`}><Icons.close size={15}/></button>}
      </div>
      <div className="flex flex-wrap items-center gap-2" aria-label={`${selectedLabel} filters`}>
        {statuses.length > 0 && <AdminSelect value={status} onChange={(event) => { setStatus(event.target.value); }} aria-label={`Filter ${selectedLabel} by status`}><option value="all">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</AdminSelect>}
        {secondaryFilter}
        <AdminSelect value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); }} aria-label={`Rows per page for ${selectedLabel}`}><option value="5">5 / page</option><option value="10">10 / page</option><option value="25">25 / page</option></AdminSelect>
      </div>
    </div>
    ...
    <div className="table-scroll" aria-busy={remote?.loading}>
      {remote?.loading && <p role="status" className="p-3 text-sm text-slate-600">Loading records…</p>}
    ...
    <Pagination page={currentPage} pages={pages} count={total} from={total ? start + 1 : 0} to={Math.min(start + pageSize, total)} onPage={setPage}/>
  </div>;
}
