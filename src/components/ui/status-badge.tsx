...
const tones: Record<string, string> = {
  ...
  "Under review": "bg-amber-50 text-amber-800 ring-amber-600/20",
  "Not verified": "bg-slate-100 text-slate-600 ring-slate-500/20",
  Suspended: "bg-red-50 text-red-700 ring-red-600/20",
  Rejected: "bg-red-50 text-red-700 ring-red-600/20",
  Removed: "bg-red-50 text-red-700 ring-red-600/20",
  Failed: "bg-red-50 text-red-700 ring-red-600/20",
  Cancelled: "bg-slate-100 text-slate-600 ring-slate-500/20",
  Open: "bg-orange-50 text-orange-800 ring-orange-600/20",
  Expired: "bg-slate-100 text-slate-600 ring-slate-500/20",
  Draft: "bg-slate-100 text-slate-600 ring-slate-500/20",
  Paused: "bg-violet-50 text-violet-700 ring-violet-600/20",
  Sold: "bg-cyan-50 text-cyan-800 ring-cyan-600/20",
};

export function StatusBadge({ status }: { status: Status | string }) {
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ring-inset ${tones[status] ?? tones.Resolved}`}
    >
      {status}
    </span>
  );
